...
*   `scope`: the OAuth 2.0 scopes to include in the resulting STS access token.
...
The response contains an ID token and an STS access token. The STS access token is issued
